...
export class FilesService {
  ...
  async getDirectoryStructure(relativePath: string = ""): Promise<FileNode[]> {
    try {
      ...
      throw new FileException(`Failed to read directory structure: ${err.message}`);
    }
  }
  ...
  async getFileContent(module: string, filename: string): Promise<string> {
    try {
      ...
      const err = error as Error;
      throw new FileException(`Failed to read file ${filename}: ${err.message}`);
    }
  }
  ...
  async getFileMetadata(module: string, filename: string) {
    try {
      ...
      const err = error as Error;
      throw new FileException(`Failed to get metadata for ${filename}: ${err.message}`);
    }
  }
}
